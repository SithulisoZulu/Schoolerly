...
import { GetAllCourseLevels, GetCoursesByCategoryId, GetCoursesByLevelId } from "../../controllers/public/course.js"
...
const getAllCourses = async() => {
    const courses = await GetCoursesByLevelId(Id)
    ...
    `
        <div class="card border bg-transparent rounded-3 mt-5 mb-5 text-white-50 w-100" id="noCourses">
            There are currently no courses under this Level                                                                                                                 
        </div>
    `
...
const getAllCourseLevels = async () => {
    ...
    async function loadLevels(lev) {
        const courses = await GetCoursesByLevelId(lev.id)
        const levels  = `
            <li class="list-inline-item mb-2">
